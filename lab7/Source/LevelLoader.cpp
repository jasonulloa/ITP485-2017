...
{
	PROFILE_SCOPE(LevelLoader_Load);
	...
	if (world.IsObject())
	{
		PROFILE_SCOPE(LevelLoader_Load_WorldProperties);
		Vector3 ambient;
	...
	if (actors.IsArray())
	{
		PROFILE_SCOPE(LevelLoader_Load_ActorSetup);
		for (rapidjson::SizeType actorIndex = 0; actorIndex < actors.Size(); ++actorIndex)
		{
			...
			{
				PROFILE_SCOPE(LevelLoader_Load_ActorSetup_UpdatedComp);
				for (rapidjson::SizeType compIndex = 0; compIndex < updatedComp.Size(); ++compIndex)
				{
					...
					if (nullptr != compPtr)
					{
						PROFILE_SCOPE(LevelLoader_Load_ActorSetup_UpdatedComp_SetProperties);
			...
			if (newComp.IsArray())
			{
				PROFILE_SCOPE(LevelLoader_Load_ActorSetup_NewComp);
...
{
	PROFILE_SCOPE(LevelLoader_GetFloatFromJSON);
...
{
	PROFILE_SCOPE(LevelLoader_GetVectorFromJSON);
	auto itr = inObject.FindMember(inProperty);
	if (itr == inObject.MemberEnd())
	{
		return false;
...
{
	PROFILE_SCOPE(LevelLoader_GetQuaternionFromJSON);
	auto itr = inObject.FindMember(inProperty);
	if (itr == inObject.MemberEnd())
	{
		return false;
