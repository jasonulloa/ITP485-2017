...
{
	// TODO Lab 5d
	Vector3 from = mOwner.GetPosition();
	Vector3 to = from;
	from.z += mFootCastOffset;
	to.z -= mFootCastOffset;
	if (mOwner.GetGame().GetPhysWorld().SegmentCast(mOwner, from, to, outComp, outPos))
	{
		return outComp->GetIsWalkable();
	}
	return false;
}
