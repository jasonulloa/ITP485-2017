	...
	,mShaderType(EMS_Basic)
{
	...
	mBoundingBox.mMax = Vector3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
}
...
bool Mesh::Load(const char* fileName, AssetCache* cache)
{
	...
	{
		PROFILE_SCOPE(Mesh_Load_ParseStream);
	...
	{
		PROFILE_SCOPE(Mesh_Load_LoadTexures);
	...
	{
		PROFILE_SCOPE(Mesh_Load_ReadVertices);
	...
	{
		PROFILE_SCOPE(Mesh_Load_LoadVertices);
	...
	{
		PROFILE_SCOPE(Mesh_Load_LoadIndicies);
