...
public:
	...
	void SetPosition(const Vector3& pos) { PROFILE_SCOPE(Actor_SetPosition); mPosition = pos; ComputeWorldTransform(); }
	const Vector3& GetPosition() const { return mPosition; }

	void SetScale(float scale) { PROFILE_SCOPE(Actor_SetScale); mScale = scale; ComputeWorldTransform(); }
	float GetScale() const { return mScale; }

	void SetRotation(const Quaternion& rotation) { PROFILE_SCOPE(Actor_SetRotation); mRotation = rotation; ComputeWorldTransform(); }
	const Quaternion& GetRotation() const { return mRotation; }

	const Matrix4& GetWorldTransform() { PROFILE_SCOPE(Actor_GetWorldTransform); return mWorldTransform; }
