...
ID3D11Texture2D* GraphicsDriver::CreateOffscreenTexture(GraphicsTexturePtr &outTexture, int inWidth, int inHeight) {
	D3D11_TEXTURE2D_DESC desc;
	ZeroMemory(&desc, sizeof(desc));
	...
	HRESULT hr = g_pd3dDevice->CreateTexture2D(&desc, nullptr, &pOffscreenTexture);

	ID3D11ShaderResourceView* toRet = nullptr;
	hr = g_pd3dDevice->CreateShaderResourceView(pOffscreenTexture, NULL, &toRet);

	outTexture = GraphicsTexturePtr(toRet, AutoReleaseD3D);

	return pOffscreenTexture;
}

RenderTargetPtr GraphicsDriver::CreateOffscreenRenderTarget(ID3D11Texture2D* inTexture) {
	ID3D11RenderTargetView* toRenderTarget;
	HRESULT hr = g_pd3dDevice->CreateRenderTargetView(inTexture, nullptr, &toRenderTarget);
	inTexture->Release();
	if (FAILED(hr))
	{
		return nullptr;
	...
	return RenderTargetPtr(toRenderTarget, AutoReleaseD3D);
}
...
void GraphicsDriver::SetOffscreenRenderTarget1(RenderTargetPtr inTarget) {
	mOffscreenRenderTarget1 = inTarget;
}

void GraphicsDriver::SetOffscreenRenderTarget2(RenderTargetPtr inTarget) {
	mOffscreenRenderTarget2 = inTarget;
}

void GraphicsDriver::SetHalfRenderTarget(RenderTargetPtr inTarget) {
	mHalfRenderTarget = inTarget;
}

void GraphicsDriver::SetQuarterRenderTarget1(RenderTargetPtr inTarget) {
	mQuarterRenderTarget1 = inTarget;
}

void GraphicsDriver::SetQuarterRenderTarget2(RenderTargetPtr inTarget) {
	mQuarterRenderTarget2 = inTarget;
}
...
BlendStatePtr GraphicsDriver::CreateAdditiveBlendState()
{
	D3D11_BLEND_DESC desc;
	ZeroMemory(&desc, sizeof(desc));
	...
	ID3D11BlendState* toRet;
	auto hr = g_pd3dDevice->CreateBlendState(&desc, &toRet);
	DbgAssert(hr == S_OK, "Problem Creating Additive Blend State");
