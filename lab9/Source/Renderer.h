...
public:
	...
	void DrawHDRSprite(TexturePtr texture, float exposure, const Matrix4& worldTransform);
	void DrawBloomMaskSprite(TexturePtr texture, const Matrix4& worldTransform);
	void DrawHorizontalBlurSprite(TexturePtr texture, float width, const Matrix4& worldTransform);
	void DrawVerticalBlurSprite(TexturePtr texture, float height, const Matrix4& worldTransform);
	void DrawMesh(VertexArrayPtr vertArray, std::vector<TexturePtr> texture, const Matrix4& worldTransform, Vector3 color, EMeshShader type = EMS_Basic);
...
private:
	...
	ShaderPtr mBloomMaskShader;
	ShaderPtr mHPassShader;
	ShaderPtr mVPassShader;
	VertexArrayPtr mSpriteVerts;

	TexturePtr mOffscreenTexture1;
	TexturePtr mOffscreenTexture2;
	TexturePtr mHalfTexture;
	TexturePtr mQuarterTexture1;
	TexturePtr mQuarterTexture2;
