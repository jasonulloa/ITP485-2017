...
{
	mGlobalInvBindPoses.emplace_back(Matrix4::Identity);

	for (int i = 1; i < mBones.size(); i++) {
		mGlobalInvBindPoses.emplace_back(mBones[i].mLocalBindPose.ToMatrix() * mGlobalInvBindPoses[mBones[i].mParent]);
	}

	for (int i = 1; i < mGlobalInvBindPoses.size(); i++) {
