...
void Animation::GetGlobalPoseAtTime(std::vector<Matrix4>& outPoses, SkeletonPtr inSkeleton, float inTime)
{
	...
	int frameBefore = inTime / timePerFrame;
	int frameAfter;
	if (frameBefore < mNumFrames - 1) {
		frameAfter = frameBefore + 1;
	} 
	else {
		frameAfter = frameBefore;
	}
	float percent = (static_cast<float>(frameAfter) * timePerFrame - inTime) / timePerFrame;

	for (int i = 0; i < mTracks.size(); i++) {
		if (mTracks[i].size() == 0) {
			outPoses.emplace_back(Matrix4::Identity);
			continue;
		}
		outPoses.emplace_back(Interpolate(mTracks[i][frameBefore], mTracks[i][frameAfter], percent).ToMatrix());

		if (inSkeleton->GetBone(i).mParent != -1) {
			outPoses[i] *= outPoses[inSkeleton->GetBone(i).mParent];
		}
	}
}
