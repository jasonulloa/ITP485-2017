...
void BoxComponent::OnUpdatedTransform()
{
	...
	Vector3 scale = mOwner.GetScale() * mScale;
	Vector3 offset = mOwner.GetPosition();
	Vector3 newMin = mModelSpaceBounds.mMin * scale + offset;
	Vector3 newMax = mModelSpaceBounds.mMax * scale + offset;
	mWorldSpaceBounds.mMin = newMin;
	mWorldSpaceBounds.mMax = newMax;
}
