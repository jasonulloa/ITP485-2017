...
void MoveComponent::Tick(float deltaTime)
{
	...
	Vector3 vAvgVelocity = mVelocity + 0.5f * mAcceleration * deltaTime;
	pos += (vAvgVelocity + inputVelocity) * deltaTime;
	mAcceleration = mForceInternal / mMass;
	mVelocity = vAvgVelocity + 0.5f * mAcceleration * deltaTime;
...
void MoveComponent::SetProperties(const rapidjson::Value& properties)
{
	...
	float linear;
	if (GetFloatFromJSON(properties, "linearSpeed", linear)) {
		SetLinearSpeed(linear);
	}

	float horizontal;
	if (GetFloatFromJSON(properties, "horizontalSpeed", horizontal)) {
		SetHorizontalSpeed(horizontal);
	}

	float yaw;
	if (GetFloatFromJSON(properties, "yawSpeed", yaw)) {
		SetYawSpeed(yaw);
	}

	float pitch;
	if (GetFloatFromJSON(properties, "pitchSpeed", pitch)) {
		SetPitchSpeed(pitch);
	}
}
